...
def GRU_Hardware(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
    ...
    dma1.sendchannel.wait() # Waiting until the last flag is HIGH
    dma1.recvchannel.wait()
	
	# Post-processing data so that the application can continue as though it never used hardware
    gi = torch.tensor(out_stream[0:3*dim_mv]).reshape(1,3*dim_mv)
    gh = torch.tensor(out_stream[3*dim_mv:]).reshape(1,3*dim_mv)
    return gi, gh
...
def GRUCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
    ...
    # Invoking hardware 
    gi, gh = GRU_Hardware(input, hidden, w_ih, w_hh, b_ih, b_hh)
